...
class test_check_median(unittest.TestCase):
    ...
    def test_good_median(self):
        ...
        self.assertEqual(msg, "Median value of TEST was 2.0, which seems sensible.")
...
def test_filter_alleles():
    a = pd.Series(["AC", "AG", "CA", "CT", "GA", "GT", "TC", "TG", "DI", "AAT", "RA"])
...
class test_end_to_end(unittest.TestCase):
    ...
    def setUp(self):
        self.args = munge.parser.parse_args("")
        self.args.sumstats = "tests/munge_test/sumstats"
        self.args.out = "asdf"
    ...
    def test_merge_alleles(self):
        self.args.merge_alleles = "tests/munge_test/merge_alleles"
        x = munge.munge_sumstats(self.args, p=False)
        correct = pd.read_csv(
