...
class Hsq(LD_Score_Regression):
    ...
    def write_h2_results(self, output: str) -> None:
        ...
        with open(output_path, "w", encoding="utf-8") as h2_output_file:
            h2_output_file.write(
                "h2\th2_stderr\tlambda_gc\tmean_chi2\tintercept_constrained\tintercept\intercept_stderr\tscale\tcategories\tsnp_proportion\th2g_proportion\tenrichment\tcoefficients\tcoefficients_stderr\tratio\tratio_stderr\n"
            )
            h2_output_file.write(
                f"{self.output_data.get('h2', 'N/A')}\t{self.output_data.get('h2_stderr', 'N/A')}\t{self.output_data.get('lambda_gc', 'N/A')}\t{self.output_data.get('mean_chi2', 'N/A')}\t{self.output_data.get('constrained', 'N/A')}\t{self.output_data.get('intercept', 'N/A')}\t{self.output_data.get('intercept_stderr', 'N/A')}\t{self.output_data.get('scale', 'N/A')}\t{self.output_data.get('categories', 'N/A')}\t{self.output_data.get('snp_proportion', 'N/A')}\t{self.output_data.get('h2g_proportion', 'N/A')}\t{self.output_data.get('enrichment', 'N/A')}\t{self.output_data.get('coefficients', 'N/A')}\t{self.output_data.get('coefficients_stderr', 'N/A')}\t{self.output_data.get('ratio', 'N/A')}\t{self.output_data.get('ratio_stderr', 'N/A')}\n"
            )
    ...
    def summary(self, ref_ld_colnames=None, P=None, K=None, overlap=False) -> None:
        ...
        if self.constrain_intercept:
            ...
            logger.info(f"Intercept: {intercept} ({intercept_stderr})")
            if self.mean_chisq > 1:
                if self.ratio < 0:
                    logger.info("Ratio < 0 (usually indicates GC correction).")
                    self.output_data["ratio_stderr"] = "N/A"
                else:
                    logger.info(f"Ratio: {s(self.ratio)} ({s(self.ratio_se)})")
...
class RG(object):
    ...
    def summary(self) -> None:
        ...
        if self._negative_hsq:
            ...
            logger.warning(
                "WARNING: One of the h2's was out of bounds. This usually indicates a data-munging error or that h2 or N is low."
            )
