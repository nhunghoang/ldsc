...
def filter_info(info, args):
    ...
    if bad_info > 0:
        ...
        logger.info(f"WARNING: {bad_info} SNPs had INFO outside of [0,1.5]. The INFO column may be mislabeled.")
...
def parse_dat(dat_gen, convert_colname, merge_alleles, args):
    ...
    logger.info(f"Reading sumstats from {args.sumstats} into memory {args.chunksize} SNPs at a time.")
    ...
    for block_num, dat in enumerate(dat_gen):
        ...
        if len(wrong_types) > 0:
            raise ValueError(
                f"Columns {wrong_types} are expected to be numeric"
            )
...
def process_n(dat, args):
    ...
    if "N" in dat.columns:
        ...
        new = len(dat)
        logger.info(
            f"Removed {old-new} SNPs with N < {n_min} ({new} SNPs remain)."
        )
...
def munge_sumstats(args):
    ...
    logger.info(f"Writing summary statistics for {len(dat)} SNPs ({dat.N.notnull().sum()} with nonmissing beta) to {out_fname}.")
