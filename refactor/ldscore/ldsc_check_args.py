...
def check_args(args: list[Any]) -> None:
    ...
    if function_args.get("cts_bin") is not None and function_args.get("extract") is not None:
        raise argparse.ArgumentError("--cts-bin and --extract are currently incompatible.")
    if function_args.get("annot") is not None and function_args.get("cts_bin") is not None:
        raise argparse.ArgumentError("--annot and --cts-bin are currently incompatible.")
    if (function_args.get("cts_bin") is not None) != (function_args.get("cts_breaks") is not None):
    ...
    if (function_args.get("samp_prev") is not None) != (function_args.get("pop_prev") is not None):
        raise argparse.ArgumentError(
            "Must set both or neither of --samp-prev and --pop-prev."
        )

    if not function_args.get("overlap_annot") or function_args.get("not_M_5_50"):
        if function_args.get("frqfile") is not None or function_args.get("frqfile_chr") is not None:
