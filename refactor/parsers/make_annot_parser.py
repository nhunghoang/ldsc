...
class MakeAnnotconfig(ParserConfig):
    ...
    @staticmethod
    def configure_parser(parser: argparse.ArgumentParser, parent_parser: argparse.ArgumentParser = None) -> argparse.ArgumentParser:
        """Add the appropriate flags and subcommands to the ldsc parser"""

        parser.add_argument(
            "--gene-set-file", type=Path, help="a file of gene names, one line per gene."
        )
        parser.add_argument(
            "--gene-coord-file",
            type=Path,
